...
const Widgets = () => {
    ...
    return (
        <React.Fragment>
            <Row>
                <Col xl={6}>
                    <img className="" alt="200x200" width="100%" height={280} src={sophiaverse2}></img>
                </Col>
                <Col xl={6}>
                    <Row>
                        {dashboardWidgets.map((item, key) => (
                            <Col xl={6} md={2} key={key}>
                                <Card className="card-animate" onClick={() => { item.link ? history(`/${item.link}`) : console.log(item.link); }}>
                                    <CardBody>
